/*  $Id: window.pl,v 1.1 2000/03/03 13:54:17 jan Exp $
...
url(DW, URLSpec:name*) :->
	...
	;   labeled_url(URLSpec, URL, Label),
	    send(DW?frame, add_history, URL),
	    (   get(DW, url, URL)
	    ->  true
	    ;   send(DW?frame, busy_cursor),
	        get_url_to_file(URL, Path),
		send(DW, slot, page_source, Path),
		send(DW, clear),
		send(DW?frame, label, URL),	% Dubious
