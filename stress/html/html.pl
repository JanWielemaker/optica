/*  $Id: html.pl,v 1.3 2002/01/22 19:54:15 jan Exp $
...
%	html_tokens(+Path, -Tokens)
%
%	Call the SP-based xml2pl library to translate an HTML file into
%	a Prolog term.  This term has format
%
%	<document>	::= document(??)	

html_tokens(Path, Tokens) :-
	load_sgml_file(Path, Tokens).
...
doc:action(form(Attrs, Content), PB, Mode) :-
	...
	emit(Content, PB, Mode),
	send(F, finished).
doc:action(input(hidden(Name, Value)), PB, _Mode) :-
	get(PB, hypered, form, @arg3?open == @on, Form),
