/*  $Id: form.pl,v 1.1 2000/03/03 13:54:17 jan Exp $
...
variable(open,		bool := @on,	get,  "Are we the `current'").
...
select_content([element(option, Attr, Content)|T], Menu) :-
	...
	send(Menu, append, new(MI, menu_item(Value, @default, Atom))),
